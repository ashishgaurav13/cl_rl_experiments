...
class DDPG(object):
    ...
    def __init__(self, num_inputs, action_space, lr = [1e-4, 1e-3], tau = [0.001, 0.001],
        ...
        self.actor = Actor(self.actor_lr, self.num_inputs, self.action_space).to(self.device)
        self.actor_target = Actor(self.actor_lr, self.num_inputs, self.action_space).to(self.device)
        self.actor_perturbed = Actor(self.actor_lr, self.num_inputs, self.action_space).to(self.device)

        self.critic = Critic(self.critic_lr, self.num_inputs, self.action_space).to(self.device)
        self.critic_target = Critic(self.critic_lr, self.num_inputs, self.action_space).to(self.device)
    ...
    def update_parameters(self):
        ...
        value_loss = torch.nn.MSELoss()(state_action_batch, targets)
        value_loss.backward()
        ...
        policy_loss = -self.critic(state_batch, mu).mean()
