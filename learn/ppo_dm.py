...
class PolicyPPO_DM(PolicyPPO):

    def snapshot(self, trajectories, trajnum = 100, multiplier = 1, step_every = 25, casenum = 3):
        assert(casenum == 3)
        ...
        for ti in tqdm.tqdm(relevant_trajectories):
            ...
            for s in t:
                ...
                action_log_ll = torch.max(torch.nn.Softmax()(action_mode))
                ...
                for pname, param in self.named_parameters(): 
                    if "critic" not in pname and param.grad is not None:
                        self.ppi[tasknum][pname].add_(torch.abs(param.grad.detach().clone()))
        ...
        for tasknum in range(num_penalties):
            ...
            for pname, param in self.actor_critic.named_parameters():
                ...
                if pname in self.actor_critic.ppi[tasknum].keys():
                    ...
                    curr_penalty += (torch.abs(param - param_n) * ppi_n).sum()
