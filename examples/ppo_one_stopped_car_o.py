    ...
    for j in range(num_updates):
        ...
        if (j+1) % log_interval == 0 and len(log_dict['r']) > 1:
            ...
            if MeanR >= solved_at:
                ...
                torch.save([ob_rms], "models/ob_rms/osco.pt")
...
parser.add_argument("-file", default = "models/one_stopped_car_o.pt",
    help = "(default file is models/one_stopped_car_o.pt)")
parser.add_argument("-discrete", default = False, action = "store_true",
    help = "when True, default file is models/one_stopped_car_o_discrete.pt")
...
num_steps = 128
log_interval = 1
# utils.log('one_stopped_car_o')

def env_fn(i):
    env = craft.OneStoppedCarO(discrete = args.discrete)
...
if args.train:
    ...
    eval_env = craft.OneStoppedCarO(discrete = args.discrete)
