    ...
    for j in range(num_updates):
        ...
        if (j+1) % log_interval == 0 and len(log_dict['r']) > 1:
            ...
            if MeanR >= solved_at:
                ...
                torch.save([ob_rms], "models/ob_rms/2sc.pt")
...
parser.add_argument("-file", default = "models/two_stopped_cars.pt",
    help = "(default file is models/two_stopped_cars.pt)")
parser.add_argument("-discrete", default = False, action = "store_true",
    help = "when True, default file is models/two_stopped_cars_discrete.pt")
...
def env_fn(i):
    env = craft.TwoStoppedCars(discrete = args.discrete)
...
if args.train:
    ...
    eval_env = craft.TwoStoppedCars(discrete = args.discrete)
