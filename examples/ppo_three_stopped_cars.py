    ...
    for j in range(num_updates):
        ...
        if (j+1) % log_interval == 0 and len(log_dict['r']) > 1:
            ...
            if MeanR >= solved_at:
                ...
                if not os.path.exists("models"): os.mkdir("models/ob_rms")
                torch.save([ob_rms], "models/ob_rms/3sc.pt")
...
parser.add_argument("-file", default = "models/three_stopped_cars_sso.pt",
    help = "(default file is models/three_stopped_cars_sso.pt)")
parser.add_argument("-discrete", default = False, action = "store_true",
    help = "when True, default file is models/three_stopped_cars_sso_discrete.pt")
...
def env_fn(i):
    env = craft.ThreeStoppedCarsSSO(discrete = args.discrete)
...
if args.train:
    ...
    eval_env = craft.ThreeStoppedCarsSSO(discrete = args.discrete)
